...
SHMSampleDelivery::~SHMSampleDelivery() { shmobj.Close(); }
...
NetworkSampleDelivery::NetworkSampleDelivery(char *address, int port,
                                             int init_time) {
...
int NetworkSampleDelivery::DeliverSample(Sample *sample) {
  ...
  this->t =
      std::thread(&NetworkSampleDelivery::RepeatSendTcp, this, sample, false);
...
void NetworkSampleDelivery::RepeatSendTcp(Sample *sample, bool retry) {
  ...
  while (attempt < max_attempts) {
    ...
    if (setsockopt(delivery_socket, SOL_SOCKET, SO_KEEPALIVE,
                   (char *)&enableKeepAlive, sizeof(enableKeepAlive)) < 0) {
    ...
    if (send(delivery_socket, sample->bytes, (int)sample->size, 0) ==
        SOCKET_ERROR) {
...
void NetworkSampleDelivery::RepeatSendTcp1(Sample *sample, bool retry) {
  ...
  if ((delivery_socket = socket(AF_INET, SOCK_STREAM, IPPROTO_IP)) ==
      SOCKET_ERROR) {
  ...
  if (connect_status == SOCKET_ERROR) {
    ...
    // }
    // else
    // FATAL("after retry connect() failed with error code : %d",
    // WSAGetLastError());
