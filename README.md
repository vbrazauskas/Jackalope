...
The generator arguments depend on your environment. On macOS you'd want to use `-G Xcode`, while for example on Windows with Visual Studio 2022 and for 64-bit build you would use `-G "Visual Studio 17 2022" -A x64`. On Linux, no generator arguments are required. When cross-compiling for Android additional cmake arguments are needed, those are the same as described in [TinyInst build instructions for Android](https://github.com/googleprojectzero/TinyInst/blob/master/Android/README.md). If you are fuzzing Android on a VM that supports shared memory via /dev/shm, `-DANDROID_TARGET=VM` enables shared memory sample delivery.
